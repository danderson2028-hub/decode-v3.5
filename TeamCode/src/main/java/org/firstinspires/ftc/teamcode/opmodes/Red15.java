...
@Autonomous
public class Red15 extends NextFTCOpMode {
    public Red15(){
        addComponents(
    ...
    private final Pose gatePose = new Pose(12,63.5, Math.toRadians(140)).mirror();
    private final Pose clearGatePose = new Pose(12,59, Math.toRadians(120)).mirror();
    private final Pose shoot2Pose = new Pose(51,93, Math.toRadians(135)).mirror();
    private final Pose intakeClosePose = new Pose(15,84, Math.toRadians(180)).mirror();
    ...
    public void buildPaths() {
        ...
        clear1 = PedroComponent.follower().pathBuilder().addPath(new BezierLine(gatePose,clearGatePose))
                ...
                .build();
        shoot3 = PedroComponent.follower().pathBuilder().addPath(new BezierCurve(clearGatePose, new Pose(48,63).mirror(),shootPose))
        ...
        clear2 = PedroComponent.follower().pathBuilder().addPath(new BezierLine(gatePose,clearGatePose))
                .setLinearHeadingInterpolation(gatePose.getHeading(), clearGatePose.getHeading())
                .build();
        shoot4 = PedroComponent.follower().pathBuilder().addPath(new BezierCurve(clearGatePose, new Pose(48,63).mirror(),shootPose))
                .setLinearHeadingInterpolation(clearGatePose.getHeading(), shootPose.getHeading())
                .build();
